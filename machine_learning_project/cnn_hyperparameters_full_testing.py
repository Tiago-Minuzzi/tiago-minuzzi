...
p = {'lr': (0.5, 5, 10),
     ...
     'second_neuron':[16,24,32],
     'third_neuron':[8,16,24],
     'dense01_neuron':[8,16,24],
     'kernel_size':[8,12,16],
     'pool_size':[3,4,6]
     'batch_size':[10,25,50],
     'epochs': [7,8,11,15],
...
def te_hype(x_train, y_train, x_val, y_val, params):
    ...
    model.add(Conv1D(filters=params['second_neuron'], kernel_size=params['kernel_size']))
    model.add(MaxPooling1D(pool_size=params['pool_size']))

    model.add(Conv1D(filters=params['third_neuron'], kernel_size=params['kernel_size']))
    model.add(MaxPooling1D(pool_size=params['pool_size']))

    model.add(Flatten())

    model.add(Dense(params['dense01_neuron'], params['activation']))
